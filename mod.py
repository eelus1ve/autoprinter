...
py_to_ru = lambda _bool: "Вкл" if _bool else "Выкл"
    ...
    def on_any_event(self, event):
        if event.event_type == "created" and not event.is_directory:
            ...
            if event.src_path[l:].split(".")[-1].upper() in ["JPG", "JPEG", "PNG", "WEBP", "GIF", "RAW", "TIFF", "PSD", "SVG", "EPS", "PDF", "AI", "CDR"]:
                work_with_image(event.src_path[l:])
...
class Checkers:
    ...
    check_printer = lambda: True if printer in [i[2] for i in win32print.EnumPrinters(3)] else False
    check_path = lambda: True if os.path.exists(path) else False
    check_test_image = lambda: True if os.path.exists(path+"\\test_back.jpg") or not test_mode else False
    check_global_area = lambda: True if global_area[0] >= worked_area[0] and global_area[0] >= sizes[0] and global_area[1] >= worked_area[1] and global_area[1] >= sizes[1] else False
    check_worked_area = lambda: True if worked_area[0] >= 0 and worked_area[0] <= global_area[0] and worked_area[0] <= global_area[1] and worked_area[1] >= 0 and worked_area[1] <= global_area[0] and worked_area[1] <= global_area[1] else False
    check_sizes = lambda: True if sizes[0] <= global_area[0] and sizes[1] <= global_area[1] else False
    flist = [check_printer, check_path, check_test_image, check_global_area, check_worked_area, check_sizes]
    def checker(n):
        try:
            Checkers.flist[n]()
    ...
    def set_sizes():
        ...
        while True:
            try:
                ...
                clear()
                print("Некоректный ввод попробуйте ещё раз.\n\n")
    flist = [set_printer, set_path, set_rgb_mode, set_save_size_mode, set_test_mode, set_global_area, set_worked_area, set_sizes]
    def main():
        global path, path_cach, printer, rgb_mode, save_size_mode, test_mode, global_area, worked_area, sizes
        clear()
        while True:
            try:
                ...
                if a in range(len(Settings.flist)):
                    Settings.flist[a]()
                elif a == len(Settings.flist):
                    clear()
                    break
                else: 
                    raise NameError
...
def start_settings():
    ...
    while True:
        ...
        if data != {'device': {'printer': '', 'path': ''}, 'image': {'mode': {'rgb': False, 'save_size': False, 'test': False}, 'global_area': [768, 1024], 'worked_area': [56, 363], 'sizes': [378, 268]}}:
            ...
            [Settings.flist[i]() for i in range(2)]
            save()
    try:
